...
```{r warning=FALSE}
Cor_mat = data[, c(3,4,5,6,8)]
chart.Correlation(Cor_mat, histogram = TRUE, pch = 19)
```

## Kilka obserwacji z danej macierzy korelacji

-   liczba lekcji jest silnie pozytywnie skorelowana z długością kursu, co oznacza, że im więcej dany kurs ma zawartości tym jest dłuższy

Możemy także zaobserwować praktyczne zerową korelację liczby subskrybentów oraz ceną kursu, co jest zaskakujące, ponieważ zazwyczaj im coś jest tańsze tym ma więcej chętnych, jednak odnosząc się do kursów nie koniecznie jest to prawdziwe

## Modelowanie danych

```{r}
model_pusty <- lm(price~-1,data = data_cleaned)
model_pelny <- lm(price~num_subscribers+num_reviews+num_lectures+content_duration, data=data_cleaned)
```

Selekcja w przód

```{r}
model1 <- step(model_pusty,scope = c(model_pusty,model_pelny),direction = 'forward')
...
```{r}
model2 <- step(model_pelny,scope = c(model_pusty,model_pelny),direction = 'backward')
```

Eliminacja dwukierunkowa - od pustego do pełnego

```{r}
model3 <- step(model_pusty,scope = c(model_pusty,model_pelny),direction = 'both')
```

Eliminacja dwukierunkowa - od pełnego do pustego

```{r}
model4 <- step(model_pelny,scope = c(model_pusty,model_pelny),direction = 'both')
```

model1 oraz model3 to ten sam model.
Podobnie model2 i model4.
...
Na podstawie wyżej ukazanych wykresów możemy przypuszczać, że w modelu nie znalazły się żadne obserwacje nietypowe.
