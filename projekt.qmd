...
Do modelu drzewa dezycyjnego wybieram zmienne **`price`**, **`num_reviews`**, **`num_lectures`**, **`level`**, **`content_duration`**, **`published_timestamp`** oraz **`subject`**.
...
```{r}
modele = rbind(result, result1)
row.names(modele) = c("Drzewo decyzyjne", "Las losowy")
