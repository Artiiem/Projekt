...
model_pelny <- lm(num_subscribers~num_reviews+price+num_lectures+content_duration + as.factor(level)+as.factor(subject)+as.factor(year)+as.factor(is_paid),data=df)
...
```{r}
data_new <- df[-c(3225,3199,2882,2696),]
model <- lm(num_subscribers ~ num_reviews + price  + subject + level + year + is_paid,data = data_new)
summary(model)
model$coefficients
```
...
library(plotly)
a <- df %>% 
  ggplot(aes(x=price_range,y=num_subscribers))+
  geom_boxplot()
ggplotly(a)
```
