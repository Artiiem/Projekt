...
Zatem ostatecznie model składa się ze zmiennej zależnej `num_subscribers` oraz zmiennych niezależnych: `num_reviews`, `price`, `subject`,`level`,`year` oraz `is_paid` .
...
Na podstawie wyżej ukazanych wykresów oraz wartości odległości cook'a możemy stwierdzić, że w modelu znajdują się obserwacje nietypowe. Są to obserwacje numer 3225,3199,2822 oraz 2696.

Jednak nie zamierzamy ich usuwać

```{r}
model2
model3 <- lm(num_subscribers ~ num_reviews + price  + subject + level + year + is_paid + price:subject,data = df)


interact_plot(model3,pred=price,modx=subject)
...
```{r}
model <- lm(num_subscribers ~ num_reviews + price  + subject + level + year + is_paid + price:subject,data = df)
summary(model)
model$coefficients
```
...
Do modelu drzewa dezycyjnego wybieram zmienne**`is_paid`**, **`price`**, **`num_reviews`**, **`num_lectures`**, **`level`**, **`content_duration`**, przekształcone cechy z **`published_timestamp`** oraz **`subject`**.

**Zmienne niezależne (features):** Oto analiza poszczególnych zmiennych w Twojej bazie danych:

-   **`course_title`**: To jest tekstowy opis kursu. Chociaż może zawierać użyteczne informacje, analiza tekstu wymaga zaawansowanych technik przetwarzania języka naturalnego i zazwyczaj nie jest bezpośrednio używana w prostych modelach drzew decyzyjnych.

-   **`is_paid`**: To binarna zmienna wskazująca, czy kurs jest płatny. Jest to ważny czynnik, który może wpływać na liczbę subskrybentów.

-   **`price`**: Cena kursu jest istotnym czynnikiem, który może wpływać na decyzję o subskrypcji. Jest to zmienna ciągła i powinna być włączona do modelu.

-   **`num_reviews`**: Liczba recenzji może być wskaźnikiem popularności kursu, co może wpływać na liczbę subskrybentów.

-   **`num_lectures`**: Ilość materiału kursu, którą można uznać za wskaźnik wartości kursu.

-   **`level`**: Poziom trudności kursu (np. początkujący, średnio zaawansowany, zaawansowany) jest kategoryczną zmienną, która może wpływać na zainteresowanie kursami wśród różnych grup subskrybentów.

-   **`content_duration`**: Czas trwania kursu, który może wpływać na zainteresowanie potencjalnych subskrybentów.

-   **`published_timestamp`**: Data publikacji kursu, która może być przekształcona na cechy czasowe (np. rok, miesiąc), co pozwoli na zbadanie trendów czasowych w zapisach na kursy.

-   **`subject`**: Tematyka kursu jest ważnym czynnikiem, który może wpływać na liczbę subskrybentów.

-   **Przekształcenia i selekcja cech:**

    -   Tekstowe i kategoryczne zmienne, takie jak **`level`** i **`subject`**, powinny zostać zakodowane na wartości liczbowe, na przykład za pomocą kodowania etykiet.

    -   Zmienna **`published_timestamp`** powinna zostać przekształcona na format daty, a następnie być może na cechy numeryczne (rok, miesiąc, dzień).
...
fit <- rpart(num_subscribers ~ ., data = data_train, method = "anova", cp = 0.0001, maxdepth = 5, minbucket = 5)  # dla regresji, bo num_subsrtibers jest zmienną ilościową, cp - im mniej tym bardziej skomplikowane drzewo
...
```{r}
df %>%  #### To dotyczy interpretacji zmiennej price w modelu, miałem małe problemy by ogarnąc o co z tym chodziło. 
  select(price_range,num_subscribers) %>% 
  group_by(price_range) %>% 
  mutate(n=n()) %>% 
  group_by(price_range,n) %>% 
  summarise_at(vars(num_subscribers),
...
df %>% 
  ...
  geom_boxplot()+
  labs(title = 'Wykresy pudełkowe zeskalowanej liczby subskrybcji', x= 'Przedział cenowy', y = 'Zeskalowana liczba subskrybcji')+
  theme_minimal()
